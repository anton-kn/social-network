    ...
    <div class="content m-4">
        <div class="position-relative h-auto">
            ...
            <div class="comment_main position-absolute top-0 end-0 border border-1 w-75 p-3">
                <h2>Редактируем книгу</h2>
                @if(session('status'))
                    <p style="color:green;">{{ session('status') }}</p>
                @endif
                <div class="title border border-1 m-2">
                    <form method="post" action="{{ route('change-book', ['id' => $book->id]) }}">
                    @csrf
                    <div class="m-3">
                        <label for="exampleFormControlInput1" class="form-label">Название книги</label>
                        <input type="text" name="title" value="{{ $book->title }}" class="form-control" id="exampleFormControlInput1"
                    ...
                    <div class="m-3">
                        ...
                        <textarea name="text" class="form-control" id="exampleFormControlTextarea1" rows="3"
                                  placeholder="Введите текст сообщения">{{ $book->text }}</textarea>
                        @error('text')
                        <div class="link-danger">
                            <strong>{{ $message }}</strong>
                        </div>
                    ...
                    <button type="submit" class="btn btn-info m-3" >Изменить</button>
                </form>
                </div>
            </div>
        </div>
